...
namespace Microsoft.AspNetCore.Http.Generated
{
    ...
    file static class GeneratedRouteBuilderExtensionsCore
    {
        ...
        [InterceptsLocation(@"/_/Program.cs", 14, 5)]
            ...
            Delegate handler)
        {
            ...
            RequestDelegateFactoryFunc createRequestDelegate = (del, options, inferredMetadataResult) =>
            {
                ...
                var handler = Cast(del, global::Microsoft.AspNetCore.Http.IResult () => throw null!);
        ...
        [InterceptsLocation(@"/_/Program.cs", 15, 5)]
            ...
            Delegate handler)
        {
            ...
            RequestDelegateFactoryFunc createRequestDelegate = (del, options, inferredMetadataResult) =>
            {
                ...
                Debug.Assert(options.EndpointBuilder.ApplicationServices != null, "ApplicationServices not found.");
                Debug.Assert(options.EndpointBuilder.FilterFactories != null, "FilterFactories not found.");
                var handler = Cast(del, global::Microsoft.AspNetCore.Http.IResult (global::SomeEnum arg0) => throw null!);
                EndpointFilterDelegate? filteredInvocation = null;
                var serviceProvider = options.ServiceProvider ?? options.EndpointBuilder.ApplicationServices;
                ...
                jsonSerializerOptions.MakeReadOnly();
                var objectJsonTypeInfo = (JsonTypeInfo<object?>)jsonSerializerOptions.GetTypeInfo(typeof(object));
                var enum_RouteOrQueryResolver = GeneratedRouteBuilderExtensionsCore.ResolveFromRouteOrQuery("enum", options.RouteParameterNames);
                ...
                if (options.EndpointBuilder.FilterFactories.Count > 0)
                {
                    filteredInvocation = GeneratedRouteBuilderExtensionsCore.BuildFilterDelegate(ic =>
                    {
                        ...
                        return ValueTask.FromResult<object?>(handler(ic.GetArgument<global::SomeEnum>(0)!));
                    },
                    options.EndpointBuilder,
                    handler.Method);
                }
                ...
                {
                    var wasParamCheckFailure = false;
                    // Endpoint Parameter: enum (Type = SomeEnum, IsOptional = False, IsParsable = True, IsArray = False, Source = RouteOrQuery)
                    var enum_raw = enum_RouteOrQueryResolver(httpContext);
                    if (enum_raw is StringValues { Count: 0 })
                    {
                        wasParamCheckFailure = true;
                        logOrThrowExceptionHelper.RequiredParameterNotProvided("SomeEnum", "enum", "route or query string");
                    }
                    var enum_temp = (string?)enum_raw;
                    if (!Enum.TryParse<global::SomeEnum>(enum_temp!, out var enum_parsed_temp))
                    {
                        if (!string.IsNullOrEmpty(enum_temp))
                        {
                            logOrThrowExceptionHelper.ParameterBindingFailed("SomeEnum", "enum", enum_temp);
                            wasParamCheckFailure = true;
                        }
                    }
                    global::SomeEnum enum_local = enum_parsed_temp!;
                ...
                async Task RequestHandlerFiltered(HttpContext httpContext)
                {
                    ...
                    var result = await filteredInvocation(EndpointFilterInvocationContext.Create<global::SomeEnum>(httpContext, enum_local!));
                    if (result is not null)
                    {
                        await GeneratedRouteBuilderExtensionsCore.ExecuteReturnAsync(result, httpContext, objectJsonTypeInfo);
                    }
